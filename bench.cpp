...
static void correctness() {
    ...
    for (int n : {1, 2, 3, 7, 15, 16, 17, 31, 32, 33, 63, 64, 65, 100, 127,
                  128, 255, 256, 1023, 1024, 2048, 4096}) {
        for (int trial = 0; trial < 50; trial++) {
            ...
            for (int q = 0; q < 200; q++) {
                ...
                if (r_pi5 != want || r_pi5_spn != want ||
                    ...
                    r_gv4 != want || r_gv4_spn != want) {
                    std::fprintf(stderr,
                                 "MISMATCH n=%d target=%u want=%d pi5=%d pi5s=%d m4=%d m4s=%d gv4=%d gv4s=%d\n",
                                 n, target, want, r_pi5, r_pi5_spn, r_m4, r_m4_spn, r_gv4, r_gv4_spn);
                    std::exit(1);
                }
                if (n == 4096) {
                    ...
                    if (r_gv4_4096 != want || r_m4_4096 != want || r_pi5_4096 != want) {
                        std::fprintf(stderr,
                ...
                if (r_intel != want || r_intel_spn != want) {
                    std::fprintf(stderr,
                                 "MISMATCH n=%d target=%u want=%d intel=%d intels=%d\n",
                                 n, target, want, r_intel, r_intel_spn);
                    std::exit(1);
                }
                if (n == 4096) {
...
int main(int argc, char **argv) {
    ...
    for (int size : {8, 16, 32, 64, 128, 256, 512, 1024, 2048, 4096}) {
        ...
        // n=4096 compile-time specialization A/B rows. Same role as the
        // x86 `4096*` row: compare each host's general-n spine against its
        // compile-time-n=4096 unroll on the same page as the general-n
        // numbers. One row per ARM variant that has a _4096 specialization.
        if (size == 4096) {
            auto q_gv4_4096 = [&](size_t i, uint16_t t) {
                return simd_quad_graviton_spine_4096(
                    arrs[i].data(), spines_gv4[i].data(), t);
            };
            double w_gv4_4096 = bench_warm(q_gv4_4096, num_arrays, targets, warm_reps);
            double c_gv4_4096 = bench_cold(q_gv4_4096, num_arrays, targets);
            std::printf(" 4096* | gv4_spine=%.2f gv4_spine_4096=%.2f  (warm)  | "
                        "gv4_spine=%.2f gv4_spine_4096=%.2f  (cold)\n",
                        w[8], w_gv4_4096, c[8], c_gv4_4096);

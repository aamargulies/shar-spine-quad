#!/usr/bin/env python3
"""Aggregate Raspberry Pi 5 bench runs (main bench + bench_twolevel).

Main bench output has 9 warm + 9 cold columns (ARM):
  linear, binary, simd(ref), pi5, pi5s, m4, m4s, gv4, gv4s
and three `4096*` A/B rows (gv4, m4, pi5) that carry the
...
def emit_pi5_dicts(agg):
    """Emit pi5_warm / pi5_cold for plot.py with 9 columns (ARM: linear, binary,
    ...
    for regime_name, regime_idx in [("pi5_warm", 0), ("pi5_cold", 1)]:
        print(f"\n{regime_name} = {{")
...
def main():
    ...
    if pi5_spec[0] is not None:
        print(f"\npi5_spine_4096 specialization:  warm={pi5_spec[0]:.2f}  cold={pi5_spec[1]:.2f}")
    if m4_spec[0] is not None:
        print(f"m4_spine_4096 on this host:     warm={m4_spec[0]:.2f}  cold={m4_spec[1]:.2f}")
    if gv4_spec[0] is not None:
        print(f"gv4_spine_4096 on this host:    warm={gv4_spec[0]:.2f}  cold={gv4_spec[1]:.2f}")

    emit_pi5_dicts(bench_agg)

    if pi5_spec[0] is not None:
        print(f"\npi5_spine_4096 = {{'warm': {pi5_spec[0]:.2f}, 'cold': {pi5_spec[1]:.2f}}}")

    if tl_paths:
        tl_agg = aggregate_twolevel(tl_paths)
        print("\n=== Pi 5 bench_twolevel medians (runs={}) ===".format(len(tl_paths)))

#!/usr/bin/env python3
"""Aggregate the 2026-05-19 F vs F* sweep on Pi 5 (Cortex-A76).

Reads pi5_runs/sweep_n${N}_${mode}_run${R}.txt (RESULT lines emitted by
bench_sweep), takes the per-cell median across the 5 runs, and prints:
    ...
    2. A "paste into plot.py" block: pi5_sweep = {N: {mode: (F, Fstar)}}.
...
def main():
    ...
    print("=== Pi 5 F vs F* sweep medians (5 runs, 100x200, 2026-05-19) ===")
    print(f"{'n':>5}  "
    ...
    print("\n# paste into plot.py (or pi5_sweep_data.py):")
    print("pi5_sweep = {")
    for n in ns:
        cells = []
        for m in MODES:
